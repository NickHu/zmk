...
static int set_cpi(const struct device *dev, uint32_t cpi) {
    ...
    uint16_t value = (cpi / 100) - 1;

    LOG_INF("Setting CPI to %u (reg value 0x%x)", cpi, value);

    // set upper byte first
    int err;
    err = reg_write(dev, PMW3389_REG_RESOLUTION_H, (value >> 8) & 0xFF);
    if (err) {
        LOG_ERR("Failed to change CPI upper");
    }
    err = reg_write(dev, PMW3389_REG_RESOLUTION_L, value & 0xFF);
    if (err) {
        LOG_ERR("Failed to change CPI lower");
    }
...
#define PMW3389_DEFINE(n)                                                                          \
    ...
    DEVICE_DT_INST_DEFINE(n, pmw3389_init, NULL, &data##n, &config##n, POST_KERNEL,                \
                          CONFIG_SENSOR_INIT_PRIORITY, &pmw3389_driver_api);
